...
fn main() -> ! {
    ...
    // TODO ClockControl::configure(clock_control, cpu_clock_speed)
    ...
    let mut sensor_power = io.pins.gpio6.into_push_pull_output();
    let _ = sensor_power.set_low();
    ...
    loop {
        ...
        let reading = sht40
            .get_temp_and_rh(Precision::High, TempUnit::MilliDegreesFahrenheit)
            .unwrap();
        let _ = sensor_power.set_low();
        println!("Got reading: {:?}", reading);
        println!("Sending temperature data to server");

        let result = socket.open(IpAddress::Ipv4(Ipv4Address::new(10, 42, 0, 1)), 4000);
        println!("socket open: {:?}", result);
